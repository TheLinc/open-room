...
describe('pathWithin', () => {
  it('makes an absolute path inside the checkout relative, posix-style', () => {
    expect(pathWithin('C:\\wt\\x', 'C:\\wt\\x\\src\\a.ts')).toBe('src/a.ts')
    expect(pathWithin('/wt/x', '/wt/x/src/a.ts')).toBe('src/a.ts')
  })

  it('keeps a relative path relative to the checkout', () => {
    expect(pathWithin('/wt/x', 'src/a.ts')).toBe('src/a.ts')
  })

  it('refuses paths that leave the checkout, and the checkout itself', () => {
    expect(pathWithin('/wt/x', '/wt/y/a.ts')).toBeNull()
    expect(pathWithin('/wt/x', '../y/a.ts')).toBeNull()
    expect(pathWithin('/wt/x', '/wt/x')).toBeNull()
  })
})
...
describe('fileDiff', () => {
  it('diffs a tracked file against HEAD in the workspace', async () => {
    ...
    const result = await fileDiff(git, '/ws', { kind: 'head' }, '/ws/a.ts')
  ...
  it('diffs against the recorded base commit for a worktree conversation', async () => {
    const { git, calls } = fake(true)
    await fileDiff(git, '/wt/x', { kind: 'branch-base', commit: 'abc123' }, 'a.ts')
  ...
  it('shows an untracked file as all additions via --no-index', async () => {
    ...
    const result = await fileDiff(git, '/ws', { kind: 'head' }, 'new.ts')
  ...
  it('reports an oversized diff instead of shipping it to the renderer', async () => {
    ...
    const result = await fileDiff(git, '/ws', { kind: 'head' }, 'big.ts')
  ...
  it('reports a binary file', async () => {
    ...
    const result = await fileDiff(git, '/ws', { kind: 'head' }, 'l.png')
    expect(result).toMatchObject({ ok: true, binary: true })
  })

  it('says when there is nothing to show', async () => {
    const { git } = fake(true, '')
    expect(await fileDiff(git, '/ws', { kind: 'head' }, 'a.ts')).toMatchObject({
      ok: true,
      empty: true
    })
  })
